...
PDokanFCB DokanGetFCB(__in PDokanVCB Vcb, __in PWCHAR FileName, __in ULONG FileNameLength, BOOLEAN CaseSensitive)
{
...
NTSTATUS DokanGetParentDir(__in const WCHAR *fileName, __out WCHAR **parentDir, __out ULONG *parentDirLength)
{
...
LONG DokanUnicodeStringChar(__in PUNICODE_STRING UnicodeString, __in WCHAR Char)
{
    ULONG i = 0;
...
NTSTATUS
DokanCheckShareAccess(_In_ PFILE_OBJECT FileObject, _In_ PDokanFCB FcbOrDcb, _In_ ACCESS_MASK DesiredAccess,
...

{
    ...
    //
    if ((FcbOrDcb->Identifier.Type == FCB) && !FlagOn(ShareAccess, FILE_SHARE_WRITE) &&
        FlagOn(DesiredAccess,
               FILE_EXECUTE | FILE_READ_DATA | FILE_WRITE_DATA | FILE_APPEND_DATA | DELETE | MAXIMUM_ALLOWED) &&
...
--*/
{
    ...
    __try
    {
        ...
        if ((fileObject->FileName.Length > sizeof(WCHAR)) && (fileObject->FileName.Buffer[1] == L'\\') &&
            ...
            if (relatedFileObject->FsContext2)
            {
                ...
                if (relatedCcb->Fcb)
                {
                    ...
                    if (relatedFcb->FileName.Length > 0 && relatedFcb->FileName.Buffer != NULL)
                    {
                        ...
                        relatedFileName->Buffer = ExAllocatePool(relatedFcb->FileName.MaximumLength);
        ...
        if (!dcb->UseAltStream && DokanUnicodeStringChar(&fileObject->FileName, L':') != -1)
        {
        ...
        if (IS_DEVICE_READ_ONLY(DeviceObject))
        {
            ...
            if ((disposition == FILE_SUPERSEDE) || (disposition == FILE_CREATE) || (disposition == FILE_OVERWRITE) ||
                (disposition == FILE_OVERWRITE_IF) || (irpSp->Parameters.Create.Options & FILE_DELETE_ON_CLOSE))
            {
        ...
        if (irpSp->Parameters.Create.SecurityContext->AccessState)
        {
            ...
            if (irpSp->Parameters.Create.SecurityContext->AccessState->ObjectName.Length > 0)
            {
                ...
                alignedObjectNameSize = PointerAlignSize(
                    ...
                    irpSp->Parameters.Create.SecurityContext->AccessState->ObjectName.Length + sizeof(WCHAR));
            }
            ...
            {
                // add 1 WCHAR for NULL
                alignedObjectTypeNameSize = PointerAlignSize(
                    sizeof(DOKAN_UNICODE_STRING_INTERMEDIATE) +
                    irpSp->Parameters.Create.SecurityContext->AccessState->ObjectTypeName.Length + sizeof(WCHAR));
            }
        ...
        if (irpSp->Parameters.Create.SecurityContext->AccessState)
        {
            ...
            eventContext->Operation.Create.SecurityContext.AccessState.SecurityEvaluated =
                irpSp->Parameters.Create.SecurityContext->AccessState->SecurityEvaluated;
            ...
            eventContext->Operation.Create.SecurityContext.AccessState.GenerateOnClose =
                irpSp->Parameters.Create.SecurityContext->AccessState->GenerateOnClose;
            eventContext->Operation.Create.SecurityContext.AccessState.AuditPrivileges =
                irpSp->Parameters.Create.SecurityContext->AccessState->AuditPrivileges;
            eventContext->Operation.Create.SecurityContext.AccessState.Flags =
                irpSp->Parameters.Create.SecurityContext->AccessState->Flags;
            eventContext->Operation.Create.SecurityContext.AccessState.RemainingDesiredAccess =
                irpSp->Parameters.Create.SecurityContext->AccessState->RemainingDesiredAccess;
            eventContext->Operation.Create.SecurityContext.AccessState.PreviouslyGrantedAccess =
                irpSp->Parameters.Create.SecurityContext->AccessState->PreviouslyGrantedAccess;
            eventContext->Operation.Create.SecurityContext.AccessState.OriginalDesiredAccess =
            ...
            {
                eventContext->Operation.Create.SecurityContext.AccessState.SecurityDescriptorOffset =
                    (ULONG)(((char *)eventContext + alignedEventContextSize) -
                            (char *)&eventContext->Operation.Create.SecurityContext.AccessState);
            }

            eventContext->Operation.Create.SecurityContext.AccessState.UnicodeStringObjectNameOffset =
                (ULONG)(((char *)eventContext + alignedEventContextSize + securityDescriptorSize) -
                        (char *)&eventContext->Operation.Create.SecurityContext.AccessState);
            eventContext->Operation.Create.SecurityContext.AccessState.UnicodeStringObjectTypeOffset =
                eventContext->Operation.Create.SecurityContext.AccessState.UnicodeStringObjectNameOffset +
        ...
        eventContext->Operation.Create.FileNameOffset =
            (ULONG)(((char *)eventContext + alignedEventContextSize + securityDescriptorSize + alignedObjectNameSize +
        ...
        if (newFileSecurityDescriptor != NULL)
        {
            ...
            RtlCopyMemory((char *)eventContext + alignedEventContextSize, newFileSecurityDescriptor,
        ...
        if (irpSp->Parameters.Create.SecurityContext->AccessState)
        {
            ...
            // Object type name
            intermediateUnicodeStr =
                (PDOKAN_UNICODE_STRING_INTERMEDIATE)((char *)intermediateUnicodeStr + alignedObjectNameSize);
        ...
        RtlCopyMemory(((char *)&eventContext->Operation.Create + eventContext->Operation.Create.FileNameOffset),
                      parentDir ? fileName : fcb->FileName.Buffer, parentDir ? fileNameLength : fcb->FileName.Length);
        *(PWCHAR)((char *)&eventContext->Operation.Create + eventContext->Operation.Create.FileNameOffset +
...
//
        ...
        if (fcb->FileCount > 1)
        {
            ...
            status =
                DokanCheckShareAccess(fileObject, fcb, eventContext->Operation.Create.SecurityContext.DesiredAccess,
            ...
            if (!NT_SUCCESS(status))
            {
                    ...
                    !FlagOn(irpSp->Parameters.Create.Options, FILE_COMPLETE_IF_OPLOCKED))
                {
                    ...
                    DokanFCBUnlock(fcb);
                    OplockBreakStatus = FsRtlOplockBreakH(oplock, Irp, 0, eventContext,
                    ...
                    {
                        status = DokanCheckShareAccess(fileObject, fcb,
                                                       eventContext->Operation.Create.SecurityContext.DesiredAccess,
                                                       eventContext->Operation.Create.ShareAccess);
        ...
        else
        {
            IoSetShareAccess(eventContext->Operation.Create.SecurityContext.DesiredAccess,
                             eventContext->Operation.Create.ShareAccess, fileObject, &fcb->ShareAccess);
        }
        ...
        if (fcb->FileCount > 1)
        {
            status = FsRtlCheckOplock(DokanGetFcbOplock(fcb), Irp, eventContext, NULL /* DokanOplockComplete */,
                                      DokanPrePostIrp);
        ...
        status = FsRtlCheckOplockEx(DokanGetFcbOplock(fcb), Irp, OPLOCK_FLAG_OPLOCK_KEY_CHECK_ONLY, NULL, NULL, NULL);
...
// AbnormalTermination() returns true
        ...
        if (!NT_SUCCESS(status))
        {
            if (BackoutOplock)
            {
                FsRtlCheckOplockEx(DokanGetFcbOplock(fcb), Irp, OPLOCK_FLAG_BACK_OUT_ATOMIC_OPLOCK, NULL, NULL, NULL);
            }
...
VOID DokanCompleteCreate(__in PIRP_ENTRY IrpEntry, __in PEVENT_INFORMATION EventInfo)
{
    ...
    if ((info == FILE_DOES_NOT_EXIST) && (IS_DEVICE_READ_ONLY(irpSp->DeviceObject)))
    {
    ...
    if (NT_SUCCESS(status) && (irpSp->Parameters.Create.Options & FILE_DIRECTORY_FILE ||
